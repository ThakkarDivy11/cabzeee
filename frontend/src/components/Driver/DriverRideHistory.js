...
const DriverRideHistory = () => {
  ...
  const fetchRideHistory = async () => {
    ...
    try {
      ...
      const response = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:5000') + '/api/rides/driver-rides', {
        headers: {
      ...
      const mockRides = [
        ...
        {
          _id: '2',
          rider: { name: 'Jane Smith', rating: 4.9 },
        ...
        {
          _id: '3',
          rider: { name: 'Mike Johnson', rating: 4.7 },
          pickupLocation: { address: '555 Station Road, City' },
          dropLocation: { address: '777 Airport Road, City' },
          fare: 450,
          status: 'completed',
          createdAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(),
          completedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000 + 25 * 60 * 1000).toISOString()
        }
      ];
      setRides(mockRides);
    } finally {
                    ...
                    <div className="flex items-center mt-3 pt-3 border-t">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center">
                          ...
                          <span className="ml-1 text-xs text-gray-600">{ride.rider.rating}</span>
                        </div>
                      </div>
                    </div>
                  )}
