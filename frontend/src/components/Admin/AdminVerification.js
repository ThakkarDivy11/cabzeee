            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {["license", "insurance", "registration"].map((type) => {
                ...
                return (
                  <div
                    key={type}
                    className="border p-4 rounded bg-gray-100/50 backdrop-blur-sm transition-all hover:bg-gray-100"
                  >
                    <div className="flex justify-between items-center mb-3">
                      <h4 className="font-bold capitalize text-gray-700">
                        {type}
                      </h4>
                      {doc?.verified ? (
                        <span className="bg-green-500 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-sm">
                          Verified
                        </span>
                      ) : doc?.fileUrl ? (
                        <span className="bg-orange-500 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded animate-pulse shadow-sm">
                          Pending
                        </span>
                      ) : (
                        <span className="bg-gray-400 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded shadow-sm">
                          Missing
                        </span>
                      )}
                    </div>
                    ...
                    {doc?.fileUrl ? (
                      <div className="space-y-3">
                        ...
                        <div className="relative group">
                          {doc.fileUrl.toLowerCase().endsWith(".pdf") ? (
                            <div className="w-full h-32 bg-red-50 rounded border border-red-100 flex flex-col items-center justify-center transition-colors group-hover:bg-red-100">
                              ...
                              <span className="text-[10px] font-bold text-red-700 mt-2 uppercase tracking-tighter">
                                PDF Document
                              </span>
                            </div>
                          ) : (
                        ...
                        <div className="flex space-x-2 mt-4 pt-4 border-t border-gray-200">
                          ...
                          <button
                            onClick={() =>
                              handleVerify(driver._id,rejectReasons, type, false)
                            }
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setRejectReasons((prev) => ({
                                ...prev,
                                [`${driver._id}_${type}`]: e.target.value,
                              }))
                            }
                            className="w-full mt-3 p-2 text-xs rounded border border-gray-300 focus:ring-2 focus:ring-red-400 focus:outline-none"
                          />
                        </div>
                      </div>
                    ) : (
